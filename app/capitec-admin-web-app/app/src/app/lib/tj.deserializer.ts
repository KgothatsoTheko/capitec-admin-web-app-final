...
export class JsonConvert {
     ...
     */
    constructor(valueCheckingMode?: number, ignorePrimitiveChecks = false) {
    ...
    private deserializeObject_loopProperty(instance: any, classPropertyName: string, json: any): void {
        ...
        try {
            ...
            throw new Error(
                `Failed to map the JSON object to the Data Model\'${instance.constructor['name']}\' because of a type error.\n\n
    ...
    private getClassPropertyMappingOptions(instance: any, propertyName: string): MappingOptions | null {
        ...
        const mappings: any = instance[Settings.MAPPING_PROPERTY];
     ...
     * @throws throws an expection in case of failure
     */
    private verifyProperty(expectedJsonType: any, value: any, serialize = false): any {
        // Map immediately if we don't care about the type
        const invalidDate = '\tReason: Expected type is Date. Got invalid date ';
        const nullValue = '\tReason: Given value is null.';
        ...
        if (expectedJsonType instanceof Array === false && value instanceof Array === false) {
            ...
            if (typeof (expectedJsonType) !== 'undefined'
                ...
                if (!value) {
                    throw new Error(invalidDate + JSON.stringify(value));
                }
                try {
        ...
        if (expectedJsonType instanceof Array) {
            if (value === null) {
                if (this.valueCheckingMode !== ValueCheckingMode.DISALLOW_NULL) {
                    return null;
                } else { throw new Error(nullValue); }
            }
