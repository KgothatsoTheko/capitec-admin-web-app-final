...
export class idbUtil {
    ...
    async connect() {
        ...
        const requestPromise = new Promise<any>((resolve, reject) => {
            ...
            request.onupgradeneeded = async (event: any) => {
                this.idbInstance = request.result; // connecting to the database
                const prevVersion = event.oldVersion;

                const storeObjsByVersions = this.indexedDBconfig.objectStoresMetaByVersion.filter((item) => item.version > prevVersion);
                ...
                for (const item of storeObjsByVersions) {
                    for (const store of item.ObjectStoreMetadataArr) {
                        if (store.operation === StoreOperation.CREATE) {
                            ...
                            // create indexes
                            for (const indexObj of store.storeIndexes) {
    ...
    async add(insertData: Array<{ objectStoreName: string; record: Object }>, transactionObj?: IDBTransaction) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                for (const objectStoreWithInsertRecord of insertData) {
                    const itemResponse = await this.insertObject(transaction, objectStoreWithInsertRecord);
                    insertResponse.push(itemResponse);
                }
                //if and only if transaction is passed as a parameter
                if (transactionObj) {
    ...
    insertObject(transaction, objectStoreWithInsertRecord) {
        return new Promise((resolve, reject) => {
            const request = transaction.objectStore(objectStoreWithInsertRecord.objectStoreName)
    ...
    async put(updateData: Array<{ objectStoreName: string; record: Object }>, transactionObj?: IDBTransaction) {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                for (const objectStoreWithUpdateRecord of updateData) {
                    const itemResponse = await this.upsertObject(transaction, objectStoreWithUpdateRecord);
    ...
    upsertObject(transaction, objectStoreWithUpdateRecord) {
        return new Promise((resolve, reject) => {
            const request = transaction.objectStore(objectStoreWithUpdateRecord.objectStoreName)
    ...
    async remove(removeData: { objectStoreName: string; primaryKeypathValue: any }, transactionObj?: IDBTransaction) {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                const removeResponse = await this.removeObject(transaction, removeData);
                if (transactionObj) {
    ...
    removeObject(transaction, removeData) {
        return new Promise((resolve, reject) => {
            const request = transaction.objectStore(removeData.objectStoreName)
    ...
    async getCursor(objectStoreName: string, onSuccessCallback: (cursorObj: IDBRequest) => any,
        where: IDBKeyRange | string | number, keyindexName?: string, transactionObj?: IDBTransaction) {
        return new Promise((resolve, reject) => {
            let objectStore;
            if (transactionObj) {
